...
```{r}
  ...
  map2_dfr(vars, rep("post_op_comp3", times = length(vars)), sub_sum_1) %>% 
    kable()
...
```{r}
  ...
  map2_dfr(vars, rep("death_6mo", times = length(vars)), sub_sum_1) %>% 
    kable()
...
```{r}
  ...
  map2_dfr(vars, rep("comp8wk", times = length(vars)), sub_sum_1) %>% 
    kable()
...
```{r}
  ...
  map2_dfr(vars, rep("comp6m", times = length(vars)), sub_sum_1) %>% 
    kable()
...
```{r}
  ...
  quasip_1 <- function(out, var){
    form_ <- as.formula(paste0(
      out, " ~ group * rcs(", var, ", 4) + rcs(age, 4) + offset(obs_time_1_log)"
    )) 
...
```{r}
  ...
  map2_dfr(vars, rep("UNITS_30d_num", times = length(vars)), sub_sum_2) %>% 
    kable()
...
```{r}
  ...
  models <- map(baseline_labs, quasip_2, out = "BT_30d_num")
...
```{r}
  ...
  models <- map(baseline_labs_log, quasip_2, out = "BT_30d_num")

  plots <- map2(models, baseline_labs_log, plot_int_qp, lims = c(-6, 2))
...
```{r}
  ...
  map2_dfr(vars, rep("UNITS_6mo_num", times = length(vars)), sub_sum_2) %>% 
    kable()
...
```{r}
  ...
  models <- map(baseline_labs, quasip_2, out = "UNITS_6mo_num")
