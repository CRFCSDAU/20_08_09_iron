...
```{r}
  ...
  levs <- c("0.7+ mmol/L", "< 0.7 mmol/L")

  data_itt <- filter(data, !is.na(primary_30d)) %>%
    arrange(identifier) %>%
    mutate(
      phos_chg_mmolL = phos_preop_mmolL - phos_bl_mmolL, 
      phos_bl_07_mmolL = factor(ifelse(
        phos_bl_mmolL <= 0.7, "< 0.7 mmol/L", "0.7+ mmol/L"
        ), levels = levs),
      phos_preop_07_mmolL = factor(ifelse(
        phos_preop_mmolL <= 0.7, "< 0.7 mmol/L", "0.7+ mmol/L"
        ), levels = levs)
    )
...
```{r}
  ...
  table.1.var <- data_itt %>%
    ...
    select(arm = group, phos_bl_mmolL, log10_phos_bl_mmolL, phos_bl_07_mmolL, 
           phos_preop_mmolL, log10_phos_preop_mmolL, phos_preop_07_mmolL, 
           phos_chg_mmolL) 
  ...
  vars <- c(
    ...
    levels(data_itt$phos_bl_07_mmolL),
    "Pre-op phosphate (mmol/L)",
    "Pre-op phosphate (log10(mmol/L))",
    "Low pre-op phospate",
    levels(data_itt$phos_preop_07_mmolL),
...
```{r}
  ...
  m2 <- glm(phos_preop_07_mmolL ~ group + mc(phos_bl_mmolL), data = data_itt, 
            family = binomial())
```
